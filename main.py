...
def write_to_csv(
    unified_data, unified_file: str = UNIFIED_FILE
) -> None:
    """Write converted data to csv file."""
    with open(unified_file, "w", newline="\n") as csv_file:
...
if __name__ == "__main__":
    ...
    for bank_name, file_path in file_path_by_name.items():
        ...
        try:
            read_from_csv(bank_name, file_path, BANKS_CONFIG[bank_name])
    ...
    sorted_unified_data = sorted(unified_data, key=lambda item: item["date"])
    try:
        write_to_csv(sorted_unified_data)
    except Exception as exc:
